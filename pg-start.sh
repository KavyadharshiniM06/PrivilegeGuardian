...
# Usage:  sudo bash pg-start.sh [--reset] [--no-seed]
# ============================================================
set -euo pipefail
...
# ════════════════════════════════════════════════════════════
# STEP 3 — Backend: npm install + seed
# ════════════════════════════════════════════════════════════
...
if [[ "$NO_SEED" == false ]]; then
  info "Running seed (creates admin/auditor users + demo data)..."
...
# ════════════════════════════════════════════════════════════
# STEP 7 — Ingest historical auditd log (converted on-the-fly)
# ════════════════════════════════════════════════════════════
step "Ingesting historical audit events"
...
if [[ -f "$HIST_LOG" && -s "$HIST_LOG" ]]; then
  ...
  }
  {
    ...
    # Timestamp
    match($0, /msg=audit\([0-9]+/)
    ts = ""
    if (RSTART > 0) {
      ts_raw = substr($0, RSTART+10, RLENGTH-10)
      ts = ts_raw "000"   # epoch ms placeholder — just use epoch
    }
  ...
  if [[ -s "$CONVERTED" ]]; then
    TOTAL=$(wc -l < "$CONVERTED")
    info "Ingesting $TOTAL converted events via API..."
    ...
    while IFS= read -r line; do
      ...
      [[ $((INGESTED % 100)) -eq 0 ]] && echo -n "  $INGESTED/$TOTAL ingested..."$'\r'
    done < "$CONVERTED"
    echo ""
    ok "Ingested $INGESTED historical events"
  else
    warn "No relevant auditd events found in $HIST_LOG (audit rules may be new)"
  fi
else
  warn "$HIST_LOG empty or missing — only live events will be collected"
fi
...
╚══════════════════════════════════════════════════════════╝${NC}
  ...
  ${BOLD}Historical:${NC}   $INGESTED events ingested
  ...
  ${CYAN}Re-run ingestion only:${NC}
    sudo bash pg-start.sh --no-seed
"
